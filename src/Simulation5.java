...
public class Simulation5 {
    ...
    public static void main(String[] args) {
        ...
        MassiveForceHashMap forceOnBodyHashMap = new MassiveForceHashMap();
        // simulation loop
        while (true) {
            seconds++; // each iteration computes the movement of the celestial bodies within one second.

            //Kollisionen können laut Angabe ignoriert werden

            //!!! add NamedBodys !!!
            forceOnBodyHashMap.put(sun, sun.getMassCenter());
            forceOnBodyHashMap.put(earth, earth.getMassCenter());
            forceOnBodyHashMap.put(moon, moon.getMassCenter());
            forceOnBodyHashMap.put(mars, mars.getMassCenter());
            forceOnBodyHashMap.put(deimos, deimos.getMassCenter());
            forceOnBodyHashMap.put(phobos, phobos.getMassCenter());
            forceOnBodyHashMap.put(mercury, mercury.getMassCenter());
            forceOnBodyHashMap.put(venus, venus.getMassCenter());
            forceOnBodyHashMap.put(vesta, vesta.getMassCenter());
            forceOnBodyHashMap.put(pallas, pallas.getMassCenter());
            forceOnBodyHashMap.put(hygiea, hygiea.getMassCenter());
            forceOnBodyHashMap.put(ceres, ceres.getMassCenter());
            ...
            for(int i = 0; i < bodies.length; i++){
                ...
                for(int j = 0; j < bodies.length; j++){
                    Body secCurrentBody = bodies[j];
                    if(i != j){
                        Vector3 forceToAdd = currentBody.gravitationalForce(secCurrentBody);
                        forceOnBodyVec = forceOnBodyVec.plus(forceToAdd);
                    }
                }
                forceOnBodyHashMap.put(currentBody, forceOnBodyVec);
            }
            // now forceOnBody[i] holds the force vector exerted on body with index i.

            // for each body (with index i): move it according to the total force exerted on it.
            for(int i = 0; i < forceOnBodyHashMap.keyList().size(); i++){
                Massive keyBody = forceOnBodyHashMap.keyList().get(i);
                keyBody.move(forceOnBodyHashMap.get(keyBody));
            }
